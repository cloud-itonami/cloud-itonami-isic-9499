(ns memberorg.operation
  "OperationActor -- one other-membership-organization operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (MemberOrgOps-LLM) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Membership Governance
  ...
  One graph run = one other-membership-organization operation (intake
  ...
  decision to a human operator (governing-body officer). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/publish-position` ALWAYS reaches this node when the
  governor is clean -- see `memberorg.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [memberorg.memberorgopsllm :as memberorgopsllm]
...
(defn build
  ...
  `memberorg.store/Store`).
  opts:
    :advisor      -- a `memberorg.memberorgopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (memberorgopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MemberOrgOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (memberorgopsllm/-advise advisor store request)]
            {:proposal p :audit [(memberorgopsllm/trace request p)]})))

      ;; Membership Governance Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
